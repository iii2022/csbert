...
from run_classifier import _save_ckpt

import datetime
from datetime import timedelta
import time
# from transformers.models.bert.modeling_bert import FTClassifier
    ...
    def _read_pickle(cls, input_file, _type = "train"):
        ...
        def parse_synset(sysets):
            ...
            for tree in js:
                ...
                # res = [w if w is not None else "" for w in res]
                # print(res)
                seq_ = " ".join(res)
                all_res.append(seq_)
            # 5 synset tree
            
            while len(all_res) < MAX_TREE:
                all_res += ["N"]
            # do not use synset tree
            # return all_res[:MAX_TREE], js[0][0][-1]
            return "", js[0][-1]
...
def convert_examples_to_features(examples, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for startsynset_i, startsynset in enumerate(example.start_synset):
            tokens.append("[unused0]")
            sep["start_synset"].append(len(tokens)-1)
            tokens_start_synset = tokenizer.tokenize(startsynset)
            tokens += tokens_start_synset
            sep_ids += (len(tokens) - len(segment_ids)) * [startsynset_i+1]
            segment_ids += (len(tokens) - len(segment_ids)) * [0]
        
            
        # if example.start_synset != "" and example.end_synset != "":
        for endsynset_i, endsynset in enumerate(example.end_synset):
            tokens.append("[unused1]")
            sep["end_synset"].append(len(tokens)-1)
            tokens_end_synset = tokenizer.tokenize(endsynset)
            tokens += tokens_end_synset
            sep_ids += (len(tokens) - len(segment_ids)) * [endsynset_i+MAX_TREE+1]
            segment_ids += (len(tokens) - len(segment_ids)) * [0]
...
def model_field(field, input_ids, label_ids, model, sep_ids, input_mask, segment_ids):
    # slid mask
    fill_ = torch.tensor((sep_ids == field), dtype=torch.long)
    if field > 0:
        triple_ = torch.tensor((sep_ids == 0), dtype=torch.long)
        fill_ = fill_ + triple_
    # [CLS] cannot be masked in any feature
    # fill_[:,0] = input_mask[:,0]
    fill_[:,0] = 1
    ...
    # print(field, tmp_input, tmp_mask)
    # print(field)
    # print(tmp_input.cpu().numpy().tolist())
    # print(tmp_mask.cpu().numpy().tolist())
...
def main():
    ...
    parser.add_argument("--classifier_path", default=None, type=str, required=True,
                        help="Bert pre-trained model selected in the list: bert-base-uncased, "
                             "bert-large-uncased, bert-base-cased, bert-base-multilingual, bert-base-chinese.")
    ...
    ####### timer #######
    dt = datetime.datetime.now() + timedelta(hours=args.sleep, seconds=0)
    print(dt)
    while datetime.datetime.now() < dt:
        print("Left time", dt-datetime.datetime.now())
        time.sleep(10)
    ...
    if args.do_train:
        ...
        # print(args.train_batch_size, args.gradient_accumulation_steps, args.num_train_epochs)
        num_train_steps = int(
    ...
    # classifier = FTClassifier(config)
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if args.use_ft:
                    ...
                    paths.append(logits_triple)
                    for k in range(MAX_TREE):
                        logits_start = model_field(k+1, input_ids, label_ids, model, sep_ids, input_mask, segment_ids)
                        paths.append(logits_start)
                    for k in range(MAX_TREE):
                        logits_end = model_field(k+MAX_TREE+1, input_ids, label_ids, model, sep_ids, input_mask, segment_ids)
                        paths.append(logits_end)
                    # paths = [logits_triple, logits_start, logits_end]
                    # print(paths)
                    for z in range(MAX_PATH):
                        logits_path = model_field(z+MAX_TREE+MAX_TREE+1, input_ids, label_ids, model, sep_ids, input_mask, segment_ids)
                    ...
                    # output, _ = classifier(labels = label_ids, ft_tensor = f_tensor)
                    loss = output.loss
                if n_gpu > 1:
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if global_step % args.name_save_steps == 0 and global_step > 1:
                        ...
                        # _save_ckpt(args, model, tokenizer, output_dir = "{}/{}".format(args.output_dir[:-1]+"classifier", global_step))

                        if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                            model_path = find_latest_folder(args.output_dir)
                            # classifier_path = find_latest_folder(args.output_dir[:-1]+"classifier")
                            # model = AutoModelForSequenceClassification.from_pretrained(
                            #     model_path,
                            #     from_tf=bool(".ckpt" in args.model_name_or_path),
                            #     config=config,
                            ...
                            if eval_accuracy > best_model["eval_result"]:
                                ...
                                # best_model["classifier_path"] = classifier_path
    ...
    if args.do_test and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        # classifier_path = best_model["classifier_path"]
        ...
        # classifier = FTClassifier(config)
        # classifier_path.load_state_dict(torch.load(classifier_path),strict=True)
