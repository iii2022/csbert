    ...
    # try:
        for epoch_id in range(args.n_epochs):
            ...
            if epoch_id == args.refreeze_epoch:
                freeze_net(model.encoder)
            model.train()
            # https://stackoverflow.com/questions/62090674/pytorch-model-saving-error-cant-pickle-local-object
            # optimizer.state_dict()
            # torch.save([model.state_dict(), args],"1.pt")
            # c = 0
...
def eval_detail(args):
    ...
    cp_emb = [np.load(path) for path in old_args.ent_emb_paths]
    cp_emb = torch.tensor(np.concatenate(cp_emb, 1), dtype=torch.float)
